...
const createSemesterRegistrationsIntoDB = async (
  ...
  const isThereAnyUpcomingOrOngoingSemester =
    await SemesterRegistrations.findOne({
      $or: [
  ...
  if (!isAcademicSemesterExist) {
    throw new AppError(
      ...
      'The academic Semester is not found',
    );
  }

  // check if the semester is registered
  const isSemesterRegistered = await SemesterRegistrations.findOne({
    academicSemester,
  });
...
const updateSingleSemesterRegistrationsIntoDB = async (
  ...
  if (
    currentSemesterStatus === RegistrationsStatus.UPCOMING &&
    requestedSemesterStatus === RegistrationsStatus.ENDED
  ) {
    throw new AppError(
      StatusCodes.BAD_REQUEST,
  ...
  if (
    currentSemesterStatus === RegistrationsStatus.ONGOING &&
    requestedSemesterStatus === RegistrationsStatus.UPCOMING
  ) {
    throw new AppError(
      StatusCodes.BAD_REQUEST,
      `You can not directly change status from ${currentSemesterStatus} to ${requestedSemesterStatus}`,
  ...
  const result = await SemesterRegistrations.findByIdAndUpdate(id, payload, {
    new: true,
    runValidators: true,
  });
  return result;
